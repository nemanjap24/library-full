...
public interface TransactionRepository extends JpaRepository<Transaction, Long> {
    ...
    @Query("SELECT t FROM Transaction t WHERE t.user.userId = :userId AND t.book.bookId = :bookId " +
            "AND t.action = 'borrow' AND NOT EXISTS " +
            "(SELECT t2 FROM Transaction t2 WHERE t2.user.userId = :userId " +
            "AND t2.book.bookId = :bookId AND t2.action = 'return' AND t2.date > t.date)")
