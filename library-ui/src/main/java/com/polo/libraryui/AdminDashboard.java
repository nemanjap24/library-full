...
public class AdminDashboard {
    public static VBox createAdminDashboard(Stage stage, Scene previousScene, List<BookTest> books) {
        ...
        for (BookTest book : books) {
            VBox bookCard = new VBox(10);  // Spacing between elements in the card
            bookCard.setStyle(
                    "-fx-padding: 15;" +
            ...
            Label titleLabelBook = new Label("Title: " + book.getTitle());
            titleLabelBook.setStyle("-fx-font-size: 18px; -fx-font-weight: bold; -fx-text-fill: #555555; -fx-font-family: Verdana");
            ...
            bookCard.getChildren().addAll(titleLabelBook, authorLabel, copiesLabel, addCopyButton);
            booksInfoBox.getChildren().add(bookCard);
        }
        ...
        viewInventoryButton.setOnAction(e -> {
            ...
            if (booksScrollPane.isVisible()) {
                viewInventoryButton.setText("Hide Inventory");
            } else {
